-- Update the payroll calculation function to remove tax calculations and ensure simple wage calculation
...
BEGIN
    ...
    SELECT 
        ...
        0::NUMERIC as total_deductions, -- No tax deductions - only other deductions if needed
        COALESCE(
            e.hourly_rate * LEAST(COALESCE(SUM(tr.total_hours), 0), 40) + 
            e.hourly_rate * e.overtime_rate_multiplier * GREATEST(COALESCE(SUM(tr.total_hours), 0) - 40, 0),
            0
        ) as net_pay -- Net pay equals gross pay (no tax)
    FROM employees e
