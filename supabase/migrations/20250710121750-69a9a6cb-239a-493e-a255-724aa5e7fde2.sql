...
BEGIN
    ...
    SELECT COALESCE(MAX(CAST(SUBSTRING(batch_number FROM LENGTH('BATCH-' || date_prefix || '-') + 1) AS INTEGER)), 0) + 1
    INTO next_number
    FROM public.payment_batches
    WHERE batch_number ~ ('^BATCH-' || date_prefix || '-[0-9]+$');
    
    RETURN 'BATCH-' || date_prefix || '-' || LPAD(next_number::TEXT, 3, '0');
END;
$$;
