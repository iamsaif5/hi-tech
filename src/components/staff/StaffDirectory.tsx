...
import { useCreateEmployee, useUpdateEmployee } from '@/hooks/useSupabaseData';
...
const StaffDirectory = () => {
  ...
  if (filterCompany !== 'all') queryParams.append('factory', filterCompany);
  if (filterDepartment !== 'all') queryParams.append('department', filterDepartment);
  if (filterType !== 'all') queryParams.append('employee_type', filterType);
  if (ordering) queryParams.append('ordering', ordering);

  const { data: employees, isLoading, refetch: refetchStaff } = useFetch<any[]>(`staff/members/?${queryParams.toString()}`);

  const { mutateAsync: createStaff } = usePost();
  const { mutateAsync: updateStaff } = usePut();
  ...
  const handleAddNewStaff = async (e: React.FormEvent) => {
    ...
    if (!newStaffForm.first_name || !newStaffForm.last_name) {
      toast({
        variant: "destructive",
        title: "Error",
        description: "Please fill in all required fields"
  ...
  const handleEditStaff = (employee: any, e: React.MouseEvent) => {
    ...
    setEditStaffForm({
      id: employee.id,
      employee_number: employee.clock_number || '',
      ...
      atg_clock_number: employee.clock_number || '',
  ...
  const handleUpdateStaff = async (e: React.FormEvent) => {
    ...
    try {
      await updateStaff({
        ...
        data: {
          clock_number: editStaffForm.atg_clock_number,
          ...
          phone_number: editStaffForm.phone,
                  ...
                  >
                    <td className="py-2 px-4">
                      <div>
                        ...
                        {/* <p className="text-xs text-gray-500">{employee.clock_number}</p> */}
                      </div>
                    </td>
                    <td className="py-2 px-4 text-xs">{employee.clock_number || 'N/A'}</td>
                    <td className="py-2 px-4 text-xs">{employee.department || 'N/A'}</td>
                    <td className="py-2 px-4 text-xs">R{employee.hourly_rate || 0}/hr</td>
                    <td className="py-2 px-4">
                      <Badge variant="outline" className="text-xs">{employee.employee_type}</Badge>
                    </td>
                    <td className="py-2 px-4">
                      <div className="flex items-center gap-2">
                        <div className={`w-3 h-3 rounded-full ${employee.is_active !== false ? 'bg-green-500' : 'bg-gray-400'
                          }`}></div>
                ...
                <div className="flex items-center space-x-4">
                  <div>
                    ...
                    <p className="text-gray-600">
                      {selectedEmployee.clock_number} • {selectedEmployee.department || 'Unassigned'}
                    </p>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <Card>
                      <CardHeader className="pb-2">
                        <CardTitle className="text-sm font-medium text-gray-700">Clock Number</CardTitle>
                      </CardHeader>
                      <CardContent>
                        <p className="text-lg font-semibold text-gray-900">{selectedEmployee.clock_number}</p>
                      </CardContent>
                    </Card>
                    <Card>
                      <CardHeader className="pb-2">
                    ...
                    <CardContent className="space-y-4">
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-gray-900">{selectedEmployee.clock_number || 'N/A'}</p>
                        </div>
                        <div>
